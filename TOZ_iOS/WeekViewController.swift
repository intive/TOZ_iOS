...
@IBAction func didTapScheduleMorningButton(_ sender: ScheduleControl) {
    ...
    if scheduleMorningLabelCollection[index!].scheduleSelected == false {
        ...
        let alertController = UIAlertController(title: "Potwierdzasz?", message: "Zarezerwowałeś termin \(date.dataLabel) - rano", preferredStyle: .alert)
        let okAction = UIAlertAction(title: "Tak", style: UIAlertActionStyle.default) { (_ : UIAlertAction) -> Void in
            self.additionalInfoPopUp()
...
@IBAction func didTapScheduleAfternoonButton(_ sender: ScheduleControl) {
    ...
    if scheduleAfternoonLabelCollection[index!].scheduleSelected == false {
        ...
        let alertController = UIAlertController(title: "Potwierdzasz?", message: "Zarezerwowałeś termin \(date.dataLabel) - popołudnie", preferredStyle: .alert)
        let okAction = UIAlertAction(title: "Tak", style: UIAlertActionStyle.default) { (_ : UIAlertAction) -> Void in
            self.additionalInfoPopUp()
...
func additionalInfoPopUp() {
    let alertController = UIAlertController(title: "Super", message: "Pomożesz Czarkowi i spółce uratować świat", preferredStyle: .alert)
    let okAction = UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler: nil)
        ...
        case .success(let newReservationRespond):
            DispatchQueue.main.async {
                if newReservation.date == newReservationRespond.date && newReservation.timeOfDay == newReservationRespond.timeOfDay {
                    ...
                    var newReservationWithName = newReservation
                    newReservationWithName.ownerForename = BackendAuth.shared.name
                    newReservationWithName.ownerSurname = BackendAuth.shared.surname
                    self.reservations.append(newReservationWithName)
